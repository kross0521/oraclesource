-- emloyee 테이블 전체 내용 조회
SELECT
    *
FROM
    employees;
-- emloyee 테이블 first_name, last_name,job_id 만 조회
...
ORDER BY
    salary DESC,
    commission_pct DESC;
-- 연봉이 2500,3500,7000이 아니며 직업이 SA_REP 이나 ST_CLERK 인 사원 조회
SELECT
    *
FROM
    employees
...
-- 2004년도에 고용된 사원들의 last_name, hire _date 조회
-- 고용일자 오름차순 정렬
...
-- 부서가 20,50dlrh 연봉이 5000 에서 12000 범위인 사람들의 LAST_NAME, SALARY 조회
-- 연봉 오름차순 정렬
SELECT
    last_name,
    salary
...
-- LAST_NAME 에 네번째 글자가 a인 사원들의 la
-- last_name 오름차순 정렬
SELECT
    employee_id,
    last_name
...
select e1.empno, e1.last_name
from employees e1, employees e2 
where e1.last_name like '%U%' and (select e1.empno,e1.last_name from employees e1, employees e2 where e1.department_id = e2.department_id);
